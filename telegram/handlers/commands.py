...
from infrastructure import get_db, ai_engine
...
async def cmd_dashboard(message: Message, db: AsyncSession):
    """Handle /dashboard command"""
    await show_dashboard(message, db)


@router.message(Command("monitors"))
async def cmd_monitors(message: Message, db: AsyncSession):
    """Handle /monitors command"""
    await show_monitors_list(message, db, page=1)


@router.message(Command("incidents"))
async def cmd_incidents(message: Message, db: AsyncSession):
...
async def cb_dashboard(callback: CallbackQuery, db: AsyncSession):
    """Show main dashboard"""
    await callback.answer()
    await show_dashboard(callback.message, db, edit=True)


async def show_dashboard(message_or_callback, db: AsyncSession, edit: bool = False):
...
async def cb_monitors_page(callback: CallbackQuery, db: AsyncSession):
...
async def show_monitors_list(message, db: AsyncSession, page: int = 1, edit: bool = False):
    ...
    if edit and hasattr(message, 'edit_text'):
        await message.edit_text(text, parse_mode="MarkdownV2", reply_markup=keyboard)
    else:
...
async def cb_monitor_view(callback: CallbackQuery, db: AsyncSession):
...
async def cb_monitor_toggle(callback: CallbackQuery, db: AsyncSession):
...
async def cb_monitor_favorite(callback: CallbackQuery, db: AsyncSession):
...
async def cb_monitor_delete(callback: CallbackQuery, db: AsyncSession):
    """Show delete confirmation"""
    await callback.answer("⚠️ Confirm deletion")
    monitor_id = callback.data.split(":")[-1]
    
    await callback.message.edit_text(
        "*🗑️ Delete Monitor*\n\n_Are you sure you want to delete this monitor?\\? This action cannot be undone._",
        parse_mode="MarkdownV2",
...
async def cb_monitor_confirm_delete(callback: CallbackQuery, db: AsyncSession):
...
async def cb_monitor_stats(callback: CallbackQuery, db: AsyncSession):
...
async def cb_monitor_check_now(callback: CallbackQuery, db: AsyncSession, bot: Bot):
    ...
    try:
        ...
        text = f"""{status_emoji} *Check Result: {monitor.name}*

├ Status: `{status_text}`
├ Response Time: `{result.response_time_ms:.0f}ms`
├ Status Code: `{result.status_code or 'N/A'}`
└ Checked At: `{result.completed_at.strftime('%H:%M:%S UTC') if result.completed_at else 'N/A'}`
"""
        if result.error_message:
            text += f"\n❌ *Error:*\n`{MessageFormatter.escape_markdown(result.error_message[:200])}`"
...
async def process_monitor_url(message: Message, state: FSMContext):
    ...
    await message.answer(
        "*➕ Add Monitor*  _Step 3/4_\n\nEnter a friendly name for this monitor:\n\n_Example: `My Production API`_",
        parse_mode="MarkdownV2",
        reply_markup=Keyboards.cancel_button("monitors:page:1")
    )
...
async def process_monitor_interval(callback: CallbackQuery, state: FSMContext, db: AsyncSession):
    ...
    if created:
        text = MessageFormatter.success_message(
            "Monitor created",
            f"*{MessageFormatter.escape_markdown(created.name)}* is now being monitored every `{created.interval}` seconds."
        )
        await callback.message.edit_text(
            text,
            parse_mode="MarkdownV2",
...
async def cb_incidents_page(callback: CallbackQuery, db: AsyncSession):
...
async def show_incidents_list(message, db: AsyncSession, page: int = 1, edit: bool = False):
...
async def cb_incident_view(callback: CallbackQuery, db: AsyncSession):
...
async def cb_incident_resolve(callback: CallbackQuery, db: AsyncSession):
    """Resolve an incident"""
    incident_id = callback.data.split(":")[-1]
    incident_service = IncidentService(db)
...
async def cb_incident_postmortem(callback: CallbackQuery, db: AsyncSession):
    ...
    try:
        ...
        text = f"""*📄 Postmortem Report*

{MessageFormatter.escape_markdown(postmortem[:3900])}
"""
        await callback.message.answer(
            text,
            parse_mode="MarkdownV2",
...
async def cb_stats(callback: CallbackQuery, db: AsyncSession):
...
├ Active: `{incident_stats['active_incidents']}`
└ Avg Resolution: `{incident_stats['avg_resolution_time_seconds']}s`

_Stats update in real-time as monitors run._
"""
